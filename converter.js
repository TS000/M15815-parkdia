...
const mail = require('./mail.js');
        fs.readdir('/var/sftp/uploads/', function(err, files) {
            if (err) {
               console.log(err);
            } else {
               if (files.length) {
...
const glob = require('glob'),
    options = {
        ...
        cwd: '/var/sftp/uploads',

    },

    forFiles = function (err, files) {
        const csvFilePath = `/var/sftp/uploads/${files}`;
        console.log(files)
            ...
            .then((jsonObj) => {
                const data = jsonObj;
                    for (let i = 0; i < jsonObj.length; i++) {
                        ...
                        console.log(email);
                        console.log(surname);
                        mail.main(status, agent, agentName, arrivalDate, arriva$
                    }
            }).then(() => {
                let oldPath = `/var/sftp/uploads/${files}`
                let newPath = `/var/sftp/backups/${files}`
